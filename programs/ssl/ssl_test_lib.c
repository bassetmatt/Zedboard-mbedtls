...
static int dummy_entropy( void *data, unsigned char *output, xalSize_t len )
{
    xalSize_t i;
    int ret;
    (void) data;
...
int rng_get( void *p_rng, unsigned char *output, xalSize_t output_len )
{
#if defined(MBEDTLS_TEST_USE_PSA_CRYPTO_RNG)
    (void) p_rng;
...
int delayed_recv( void *ctx, unsigned char *buf, xalSize_t len )
{
...
int delayed_send( void *ctx, const unsigned char *buf, xalSize_t len )
{
    static int first_try = 1;
    int ret;
