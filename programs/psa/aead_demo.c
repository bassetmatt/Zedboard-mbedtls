...
void print_buf( const char *title, uint8_t *buf, xalSize_t len )
{
    printf( "%s:", title );
...
static void aead_info( psa_key_id_t key, psa_algorithm_t alg )
{
    ...
    xalSize_t key_bits = psa_get_key_bits( &attr );
    psa_algorithm_t base_alg = PSA_ALG_AEAD_WITH_DEFAULT_LENGTH_TAG( alg );
    xalSize_t tag_len = PSA_AEAD_TAG_LENGTH( key_type, key_bits, alg );
