...
int mbedtls_test_asn1_skip_integer( unsigned char **p, const unsigned char *end,
                                    xalSize_t min_bits, xalSize_t max_bits,
                                    int must_be_odd )
{
    xalSize_t len;
    xalSize_t actual_bits;
    unsigned char msb;
     ...
     * It is assumed here, that end >= p, which validates casting to xalSize_t. */
    TEST_ASSERT( len <= (xalSize_t)( end - *p) );
