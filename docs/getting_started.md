...
```C
void import_a_key(const uint8_t *key, xalSize_t key_len)
{
    psa_status_t status;
...
```C
void sign_a_message_using_rsa(const uint8_t *key, xalSize_t key_len)
{
    psa_status_t status;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
...
```c
void encrypt_with_symmetric_ciphers(const uint8_t *key, xalSize_t key_len)
{
    enum {
...
```c
void decrypt_with_symmetric_ciphers(const uint8_t *key, xalSize_t key_len)
{
    enum {
        block_size = PSA_BLOCK_CIPHER_BLOCK_LENGTH(PSA_KEY_TYPE_AES),
    };
...
```c
    ...
    xalSize_t expected_hash_len = PSA_HASH_LENGTH(alg);
...
```C
    ...
    xalSize_t derived_bits = 128;
    xalSize_t capacity = PSA_BITS_TO_BYTES(derived_bits);
...
```C
    ...
    xalSize_t output_size = 0;
    xalSize_t output_length = 0;
    xalSize_t tag_length = 16;
    psa_key_attributes_t attributes = PSA_KEY_ATTRIBUTES_INIT;
    psa_key_id_t key_id;
