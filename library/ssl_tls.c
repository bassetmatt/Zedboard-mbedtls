...
static int resize_buffer( unsigned char **buffer, xalSize_t len_new, xalSize_t *len_old )
{
...
static void handle_buffer_resizing( mbedtls_ssl_context *ssl, int downsizing,
                                    xalSize_t in_buf_new_len,
                                    xalSize_t out_buf_new_len )
{
    int modified = 0;
    xalSize_t written_in = 0, iv_offset_in = 0, len_offset_in = 0;
    xalSize_t written_out = 0, iv_offset_out = 0, len_offset_out = 0;
    if( ssl->in_buf != NULL )
    {
...
typedef int (*tls_prf_fn)( const unsigned char *secret, xalSize_t slen,
                           const char *label,
...
typedef int ssl_tls_prf_t(const unsigned char *, xalSize_t, const char *,
                          const unsigned char *, xalSize_t,
                          unsigned char *, xalSize_t);
...
static int tls_prf_sha256( const unsigned char *secret, xalSize_t slen,
                           const char *label,
                           const unsigned char *random, xalSize_t rlen,
                           unsigned char *dstbuf, xalSize_t dlen );
static void ssl_calc_verify_tls_sha256( const mbedtls_ssl_context *,unsigned char*, xalSize_t * );
static void ssl_calc_finished_tls_sha256( mbedtls_ssl_context *,unsigned char *, int );

#endif /* MBEDTLS_SHA256_C */

#if defined(MBEDTLS_SHA384_C)
static int tls_prf_sha384( const unsigned char *secret, xalSize_t slen,
                           const char *label,
                           const unsigned char *random, xalSize_t rlen,
                           unsigned char *dstbuf, xalSize_t dlen );

static void ssl_calc_verify_tls_sha384( const mbedtls_ssl_context *, unsigned char*, xalSize_t * );
static void ssl_calc_finished_tls_sha384( mbedtls_ssl_context *, unsigned char *, int );
#endif /* MBEDTLS_SHA384_C */

static xalSize_t ssl_session_save_tls12( const mbedtls_ssl_session *session,
                                      unsigned char *buf,
                                      xalSize_t buf_len );
...
static void ssl_update_checksum_start( mbedtls_ssl_context *, const unsigned char *, xalSize_t );

#if defined(MBEDTLS_SHA256_C)
static void ssl_update_checksum_sha256( mbedtls_ssl_context *, const unsigned char *, xalSize_t );
#endif /* MBEDTLS_SHA256_C */

#if defined(MBEDTLS_SHA384_C)
static void ssl_update_checksum_sha384( mbedtls_ssl_context *, const unsigned char *, xalSize_t );
#endif /* MBEDTLS_SHA384_C */

int  mbedtls_ssl_tls_prf( const mbedtls_tls_prf_types prf,
                          const unsigned char *secret, xalSize_t slen,
                          const char *label,
                          const unsigned char *random, xalSize_t rlen,
                          unsigned char *dstbuf, xalSize_t dlen )
{
...
int mbedtls_ssl_conf_psk( mbedtls_ssl_config *conf,
                ...
                const unsigned char *psk_identity, xalSize_t psk_identity_len )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
int mbedtls_ssl_conf_dh_param_bin( mbedtls_ssl_config *conf,
                                   const unsigned char *dhm_P, xalSize_t P_len,
                                   const unsigned char *dhm_G, xalSize_t G_len )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
...
{
    xalSize_t max_len = MBEDTLS_SSL_IN_CONTENT_LEN;
    xalSize_t read_mfl;
...
int mbedtls_ssl_get_max_out_record_payload( const mbedtls_ssl_context *ssl )
{
    ...
    {
        const xalSize_t mtu = mbedtls_ssl_get_current_mtu( ssl );
        const int ret = mbedtls_ssl_get_record_expansion( ssl );
        const xalSize_t overhead = (xalSize_t) ret;
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    const xalSize_t mfl = mbedtls_ssl_get_input_max_frag_len( ssl );
                             ...
                             xalSize_t *olen )
{
    ...
    case MBEDTLS_SSL_MINOR_VERSION_3:
    {
        xalSize_t remaining_len = used <= buf_len ? buf_len - used : 0;
                             ...
                             xalSize_t len )
{
    ...
    if( !omit_header )
    {
        ...
        if( (xalSize_t)( end - p ) < sizeof( ssl_serialized_session_header ) )
            return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
                             ...
                             xalSize_t len )
{
     ...
     */
    if( (xalSize_t)( end - p ) < sizeof( ssl_serialized_context_header ) )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
     ...
     * Session
     */
    if( (xalSize_t)( end - p ) < 4 )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    session_len = ( (xalSize_t) p[0] << 24 ) |
                  ( (xalSize_t) p[1] << 16 ) |
                  ( (xalSize_t) p[2] <<  8 ) |
                  ( (xalSize_t) p[3]       );
    p += 4;
    ...
    if( (xalSize_t)( end - p ) < session_len )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    if( (xalSize_t)( end - p ) < sizeof( ssl->transform->randbytes ) )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    ...
    if( (xalSize_t)( end - p ) < 1 )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    ssl->transform->in_cid_len = *p++;

    if( (xalSize_t)( end - p ) < ssl->transform->in_cid_len + 1u )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    if( (xalSize_t)( end - p ) < ssl->transform->out_cid_len )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
                                            ...
                                            xalSize_t *out_len )
{
    ...
    xalSize_t named_group_list_len;     /* Length of named_group_list */
    const uint16_t *group_list = mbedtls_ssl_get_groups( ssl );
                                   ...
                                   const unsigned char *end, xalSize_t *out_len )
{
    ...
    xalSize_t supported_sig_alg_len = 0; /* Length of supported_signature_algorithms */
...
static psa_status_t setup_psa_key_derivation( psa_key_derivation_operation_t* derivation,
                                              ...
                                              psa_algorithm_t alg,
                                              const unsigned char* seed, xalSize_t seed_length,
                                              const unsigned char* label, xalSize_t label_length,
                                              xalSize_t capacity )
{
    psa_status_t status;
                               ...
                               const mbedtls_ssl_context *ssl )
{
    ...
    xalSize_t const master_secret_len = 48;
...
int mbedtls_ssl_psk_derive_premaster( mbedtls_ssl_context *ssl, mbedtls_key_exchange_type_t key_ex )
{
    ...
    if( key_ex == MBEDTLS_KEY_EXCHANGE_PSK )
    {
        ...
        if( end < p || (xalSize_t)( end - p ) < psk_len )
            return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    while( crt != NULL )
    {
        ...
        if( n > MBEDTLS_SSL_OUT_CONTENT_LEN - 3 - i )
        {
            MBEDTLS_SSL_DEBUG_MSG( 1, ( "certificate too large, %" MBEDTLS_PRINTF_SIZET
                           ...
                           i + 3 + n, (xalSize_t) MBEDTLS_SSL_OUT_CONTENT_LEN ) );
            return( MBEDTLS_ERR_SSL_BUFFER_TOO_SMALL );
        }
...
static int ssl_remember_peer_crt_digest( mbedtls_ssl_context *ssl,
                                         unsigned char *start, xalSize_t len )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
                                   ...
                                   const mbedtls_ssl_context *ssl )
{
    ...
    xalSize_t mac_key_len = 0;
    xalSize_t iv_copy_len;
    xalSize_t keylen;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info;
                                   ...
                                   xalSize_t len )
{
     ...
     * Basic mandatory fields
     */
    if( 2 + 1 + 1 + 32 + 48 + 4 > (xalSize_t)( end - p ) )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    if( 2 > (xalSize_t)( end - p ) )
        return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );

    session->peer_cert_digest_type = (mbedtls_md_type_t) *p++;
    session->peer_cert_digest_len  = (xalSize_t) *p++;
    ...
    if( session->peer_cert_digest_len != 0 )
    {
        ...
        if( session->peer_cert_digest_len > (xalSize_t)( end - p ) )
            return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
    ...
    {
        if( session->ticket_len > (xalSize_t)( end - p ) )
            return( MBEDTLS_ERR_SSL_BAD_INPUT_DATA );
