...
xalSize_t mbedtls_dhm_get_bitlen( const mbedtls_dhm_context *ctx )
{
    return( mbedtls_mpi_bitlen( &ctx->P ) );
}

xalSize_t mbedtls_dhm_get_len( const mbedtls_dhm_context *ctx )
{
...
int mbedtls_dhm_calc_secret( mbedtls_dhm_context *ctx,
                     unsigned char *output, xalSize_t output_size, xalSize_t *olen,
                     int (*f_rng)(void *, unsigned char *, xalSize_t),
                     void *p_rng )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
 ...
 */
static int load_file( const char *path, unsigned char **buf, xalSize_t *n )
{
    FILE *f;
    long size;
...
static const xalSize_t mbedtls_test_dhm_params_len = sizeof( mbedtls_test_dhm_params );
