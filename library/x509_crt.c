 ...
 */
static int x509_memcasecmp( const void *s1, const void *s2, xalSize_t len )
{
    xalSize_t i;
    unsigned char diff;
...
{
    xalSize_t i;
    xalSize_t cn_idx = 0, cn_len = strlen( cn );
...
cleanup:
    ...
    while( ( entry = readdir( dir ) ) != NULL )
    {
        ...
        if( snp_ret < 0 || (xalSize_t)snp_ret >= sizeof entry_name )
        {
...
static int x509_info_subject_alt_name( char **buf, xalSize_t *size,
...
static int x509_info_cert_type( char **buf, xalSize_t *size,
                                unsigned char ns_cert_type )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    xalSize_t n = *size;
...
static int x509_info_key_usage( char **buf, xalSize_t *size,
                                unsigned int key_usage )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    xalSize_t n = *size;
...
static int x509_info_ext_key_usage( char **buf, xalSize_t *size,
...
static int x509_info_cert_policies( char **buf, xalSize_t *size,
                        ...
                        mbedtls_x509_crt_restart_ctx *rs_ctx )
{
    ...
    for( parent = candidates; parent != NULL; parent = parent->next )
    {
        ...
        if( parent->max_pathlen > 0 &&
            (xalSize_t) parent->max_pathlen < 1 + path_cnt - self_cnt )
        {
            continue;
        }
...
static int x509_crt_check_cn( const mbedtls_x509_buf *name,
                              const char *cn, xalSize_t cn_len )
{
    /* try exact match */
    if( name->len == cn_len &&
