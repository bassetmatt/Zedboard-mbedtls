...
struct mbedtls_ecp_restart_mul
{
    ...
    xalSize_t i;               /* current index in various loops, 0 outside    */
...
static void mpi_init_many( mbedtls_mpi *arr, xalSize_t size )
{
    while( size-- )
        mbedtls_mpi_init( arr++ );
}

static void mpi_free_many( mbedtls_mpi *arr, xalSize_t size )
{
    while( size-- )
...
int mbedtls_ecp_tls_read_point( const mbedtls_ecp_group *grp,
                                mbedtls_ecp_point *pt,
                                const unsigned char **buf, xalSize_t buf_len )
{
 ...
 */
int mbedtls_ecp_tls_write_group( const mbedtls_ecp_group *grp, xalSize_t *olen,
                         unsigned char *buf, xalSize_t blen )
{
    const mbedtls_ecp_curve_info *curve_info;
    ECP_VALIDATE_RET( grp  != NULL );
...
static int ecp_normalize_jac_many( const mbedtls_ecp_group *grp,
                                   mbedtls_ecp_point *T[], xalSize_t T_size )
{
    if( T_size < 2 )
 ...
 */
static void ecp_comb_recode_core( unsigned char x[], xalSize_t d,
...
static int ecp_mul_comb_core( const mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
                              const mbedtls_ecp_point T[], unsigned char T_size,
                              const unsigned char x[], xalSize_t d,
...
int mbedtls_ecp_gen_privkey_mx( xalSize_t high_bit,
                                mbedtls_mpi *d,
                                int (*f_rng)(void *, unsigned char *, xalSize_t),
                                void *p_rng )
{
    int ret = MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
    xalSize_t n_random_bytes = high_bit / 8 + 1;
