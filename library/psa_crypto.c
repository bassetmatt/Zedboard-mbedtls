    ...
    psa_algorithm_t alg2 )
{
    ...
    {
        xalSize_t alg1_len = PSA_ALG_AEAD_GET_TAG_LENGTH( alg1 );
        xalSize_t alg2_len = PSA_ALG_AEAD_GET_TAG_LENGTH( alg2 );
        xalSize_t restricted_len = alg1_len > alg2_len ? alg1_len : alg2_len;
          ...
          PSA_ALG_FULL_LENGTH_MAC( alg2 ) ) )
    {
        ...
        xalSize_t alg1_len = PSA_MAC_LENGTH( key_type, 0, alg1 );
        xalSize_t alg2_len = PSA_MAC_LENGTH( key_type, 0, alg2 );
        xalSize_t restricted_len = alg1_len > alg2_len ? alg1_len : alg2_len;
                                      ...
                                      psa_algorithm_t requested_alg )
{
          ...
          PSA_ALG_FULL_LENGTH_MAC( requested_alg ) ) )
    {
        ...
        xalSize_t requested_output_length = PSA_MAC_LENGTH(
                                            key_type, 0, requested_alg );
        xalSize_t default_output_length = PSA_MAC_LENGTH(
                                        key_type, 0,
                                        PSA_ALG_FULL_LENGTH_MAC( requested_alg ) );
                             ...
                             mbedtls_svc_key_id_t *key )
{
    ...
    xalSize_t bits;
    xalSize_t storage_size = data_length;
...
psa_status_t psa_hash_compute( psa_algorithm_t alg,
                               ...
                               uint8_t *hash, xalSize_t hash_size,
                               xalSize_t *hash_length )
{
    *hash_length = 0;
    if( !PSA_ALG_IS_HASH( alg ) )
...
psa_status_t psa_aead_encrypt( mbedtls_svc_key_id_t key,
                               ...
                               xalSize_t additional_data_length,
...
psa_status_t psa_aead_generate_nonce( psa_aead_operation_t *operation,
                                      uint8_t *nonce,
                                      xalSize_t nonce_size,
                                      xalSize_t *nonce_length )
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
    uint8_t local_nonce[PSA_AEAD_NONCE_MAX_SIZE];
    xalSize_t required_nonce_size;
...
#if defined(PSA_WANT_ALG_GCM)
        case PSA_ALG_GCM:
            /* Lengths can only be too large for GCM if xalSize_t is bigger than 32
...
static void psa_des_set_key_parity( uint8_t *data, xalSize_t data_size )
{
    if( data_size >= 8 )
    ...
    )
{
    ...
    xalSize_t m = ecp_group.nbits;

    xalSize_t m_bytes = PSA_BITS_TO_BYTES( m );
...
{
    uint8_t *data = NULL;
    xalSize_t bytes = PSA_BITS_TO_BYTES( bits );
    xalSize_t storage_size = bytes;
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
...
static psa_status_t psa_key_agreement_ecdh( const uint8_t *peer_key,
                                            xalSize_t peer_key_length,
                                            const mbedtls_ecp_keypair *our_key,
                                            uint8_t *shared_secret,
                                            xalSize_t shared_secret_size,
                                            xalSize_t *shared_secret_length )
{
                                                ...
                                                xalSize_t peer_key_length )
{
    ...
    xalSize_t shared_secret_length = 0;
...
static psa_status_t psa_validate_key_type_and_size_for_key_generation(
    psa_key_type_t type, xalSize_t bits )
{
    psa_status_t status = PSA_ERROR_CORRUPTION_DETECTED;
