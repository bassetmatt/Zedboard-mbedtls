                                     ...
                                     xalSize_t len )
{
    ...
    xalSize_t servername_list_size, hostname_len;
                                              ...
                                              xalSize_t len )
{
    ...
    if( len == 0 || (xalSize_t)( buf[0] + 1 ) != len )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "bad client hello message" ) );
        mbedtls_ssl_send_alert_message( ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                        MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR );
...
{
    xalSize_t list_len, cur_len, ours_len;
    ...
    for( theirs = start; theirs != end; theirs += cur_len )
    {
        ...
        if( cur_len > (xalSize_t)( end - theirs ) )
        {
            mbedtls_ssl_send_alert_message( ssl, MBEDTLS_SSL_ALERT_LEVEL_FATAL,
                                            MBEDTLS_SSL_ALERT_MSG_DECODE_ERROR );
            return( MBEDTLS_ERR_SSL_DECODE_ERROR );
                                   ...
                                   xalSize_t len )
{
    ...
    xalSize_t i,j;
    xalSize_t profile_length;
    uint16_t mki_length;
    /*! 2 bytes for profile length and 1 byte for mki len */
    const xalSize_t size_of_lengths = 3;
...
static int ssl_parse_client_hello( mbedtls_ssl_context *ssl )
{
    ...
    xalSize_t i, j;
    xalSize_t ciph_offset, comp_offset, ext_offset;
    xalSize_t msg_len, ciph_len, sess_len, comp_len, ext_len;
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    xalSize_t cookie_offset, cookie_len;
#endif
                               ...
                               xalSize_t *olen )
{
    ...
    *olen = 0;
    ...
    if( end < p || (xalSize_t)( end - p ) < (unsigned)( ssl->own_cid_len + 5 ) )
    {
    ...
    */
    ...
    p += 2;
    ext_len = (xalSize_t) ssl->own_cid_len + 1;
...
static void ssl_write_alpn_ext( mbedtls_ssl_context *ssl,
                                unsigned char *buf, xalSize_t *olen )
{
                                    ...
                                    xalSize_t *olen )
{
    ...
    *olen = 0;
    ...
    if( (xalSize_t)( end - buf ) < mki_len + 9 )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "buffer too small" ) );
        return;
    }
...
static int ssl_write_certificate_request( mbedtls_ssl_context *ssl )
{
    ...
    xalSize_t ct_len, sa_len; /* including length bytes */
    unsigned char *buf, *p;
    ...
    if( ssl->conf->cert_req_ca_list ==  MBEDTLS_SSL_CERT_REQ_CA_LIST_ENABLED )
    {
        ...
        while( crt != NULL && crt->version != 0 )
        {
            ...
            if( end < p || (xalSize_t)( end - p ) < 2 + (xalSize_t) dn_size )
            {
                                           ...
                                           xalSize_t *signature_len )
{
    ...
    xalSize_t sig_max_len = ( ssl->out_buf + MBEDTLS_SSL_OUT_CONTENT_LEN
                           - sig_start );
                                            ...
                                            xalSize_t *signature_len )
{
    ...
    xalSize_t out_buf_len = ssl->out_buf_len - ( ssl->out_msg - ssl->out_buf );
#else
    xalSize_t out_buf_len = MBEDTLS_SSL_OUT_BUFFER_LEN - ( ssl->out_msg - ssl->out_buf );
#endif
#endif
...
curve_matching_done:
    ...
    {
        xalSize_t dig_signed_len = ssl->out_msg + ssl->out_msglen - dig_signed;
        xalSize_t hashlen = 0;
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
static int ssl_resume_decrypt_pms( mbedtls_ssl_context *ssl,
                                   ...
                                   xalSize_t *peer_pmslen,
                                   xalSize_t peer_pmssize )
{
    int ret = ssl->conf->f_async_resume( ssl,
                                      ...
                                      xalSize_t peer_pmssize )
{
    ...
    xalSize_t len = mbedtls_pk_get_len( public_key );
