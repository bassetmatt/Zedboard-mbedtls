...
int mbedtls_md_hmac_starts( mbedtls_md_context_t *ctx, const unsigned char *key, xalSize_t keylen )
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    ...
    if( keylen > (xalSize_t) ctx->md_info->block_size )
    {
        if( ( ret = mbedtls_md_starts( ctx ) ) != 0 )
            goto cleanup;
...
int mbedtls_md_hmac_update( mbedtls_md_context_t *ctx, const unsigned char *input, xalSize_t ilen )
{
    if( ctx == NULL || ctx->md_info == NULL || ctx->hmac_ctx == NULL )
        return( MBEDTLS_ERR_MD_BAD_INPUT_DATA );
