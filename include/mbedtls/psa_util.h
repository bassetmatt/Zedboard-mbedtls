...
static inline psa_algorithm_t mbedtls_psa_translate_cipher_mode(
    mbedtls_cipher_mode_t mode, xalSize_t taglen )
{
    switch( mode )
    {
...
static inline int mbedtls_psa_get_ecc_oid_from_id(
    psa_ecc_family_t curve, xalSize_t bits,
    char const **oid, xalSize_t *oid_len )
{
    switch( curve )
    {
...
static inline psa_key_type_t mbedtls_psa_parse_tls_ecc_group(
    uint16_t tls_ecc_grp_reg_id, xalSize_t *bits )
{
 ...
 */
typedef int mbedtls_f_rng_t( void *p_rng, unsigned char *output, xalSize_t output_size );
